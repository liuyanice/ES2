...
from .modules import GateControl2D, GateControlAtrous2D
...
class GateControlTransformer(nn.Module):
    def __init__(self,
                 d_model=512,
                 nhead=8,
        ...
        encoder_layer = GateControlTransformerEncoderLayer(
            d_model, nhead, dim_feedforward, dropout,
            activation, normalize_before)
        encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
        self.encoder = GateControlTransformerEncoder(encoder_layer,
                                                     num_encoder_layers,
                                                     encoder_norm)
...
class GateControlTransformerEncoder(nn.Module):
    def __init__(self,
                 encoder_layer,
                 num_layers,
                 norm=None):
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        src2 = self.self_attn(q,
                              ...
                              value=src,
                              attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask)[0] #MSA
...
class GateControlTransformerEncoderLayer(TransformerEncoderLayer):
    "    Add Gate-control unit to Transformer's Encoder"
    ...
    def __init__(self,
        ...
        super().__init__(d_model, nhead, dim_feedforward, dropout, activation,
                         normalize_before) 
        self.GCA = GateControlAtrous2D(128)#Or use self.GCA = GateControl2D(128)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        tgt2 = self.self_attn(q,
                              ...
                              value=tgt,
                              attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)[0] #MSA
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)
    
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt)))) #MLP
